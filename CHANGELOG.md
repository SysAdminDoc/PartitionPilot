...
- Replaced the generic partition-strip icon with a distinct storage-route identity that stays readable from the Windows title bar through a 1024-pixel brand master. Every PNG now has real alpha transparency outside the icon silhouette.
- Rebuilt the README around the decisions a storage administrator needs to make before installing: what the app changes, how it verifies a target, where evidence is stored, and which operations remain dry runs.
- Added four current screenshots from the production WPF interface with realistic simulated disks. A repeatable capture tool renders those states on a private desktop and refuses to use the interactive desktop.
- Clarified two command-bar labels. `FS Support` is now `File Systems`, and `Elevate` is now `Run as admin`; the German, Spanish, French, and pseudo-locale resources stay in sync.

### Simulation reliability

- Simulation mode no longer contacts the update channel while a screenshot or UI test is running.
- Simulated SMART data now reports a matching self-test capability instead of probing for a local `smartctl` installation and showing an irrelevant setup error.
- Added a regression test that fails if simulated health checks start an external diagnostic process again.
- Velopack now runs at the real process entry point before WPF starts. Installer and update lifecycle events no longer wait for the application startup callback.
...
- NVMe drive health no longer needs Administrator. `IOCTL_STORAGE_QUERY_PROPERTY` is defined with `FILE_ANY_ACCESS`, but the handle was opened asking for read access, and the direct NVMe read only ran as a top-up for data another provider had already returned. Unelevated, both of those providers fail with access denied, so the Disk Health tab and `pp smart` reported nothing at all on NVMe. The log page is now read with no access rights requested and used as a source in its own right. Verified on two NVMe drives from an unelevated session: temperature, wear, power-on hours, power cycles, data written and read, available spare, media errors and unsafe shutdowns all report.
- Added a SATA health path that does not depend on LibreHardwareMonitor. Drive health for SATA disks came from a third-party library or from WMI counters that need elevation, so a SATA drive often reported nothing at all. The ATA Device Statistics log is now read through the same unprivileged IOCTL the NVMe path uses, giving power-on hours, power cycles, lifetime bytes written and read, current and highest temperature, and endurance used. Statistics the drive marks unsupported are skipped rather than reported as zero. Verified unelevated against a Samsung SSD 870 EVO 4TB that previously showed no SMART data at all.
- The NVMe request now follows Microsoft's documented buffer contract: a 4144-byte buffer, `ProtocolDataOffset` measured from the start of the protocol-specific data, and the payload located through the offset and length the driver returns rather than a fixed position.

- Dropped the `CommunityToolkit.Mvvm` dependency. It supplied one base class, its upstream has had no commits since March 2026 with maintainers stating nobody is available to maintain it, and it carries an open report of `AsyncRelayCommand` crashing on .NET 10. `ViewModelBase` now implements change notification directly, with the same equality semantics, and is covered by tests.
- Bumped `System.Management` and `System.Diagnostics.PerformanceCounter` from 10.0.9 to 10.0.11 to match the current .NET servicing band. Because the app ships self-contained it carries its own runtime, so a runtime security fix reaches installed copies only when a new build is published; the release procedure now says to republish after each servicing update and to record the bundled runtime version.
- SMART queries now share one LibreHardwareMonitor instance for the life of the process instead of constructing and closing one per call. Opening one enumerates every storage device and allocates an executable code buffer for generated CPU instructions, none of which the storage-only path needs, and the temperature monitor was paying that cost once per disk every thirty seconds. The instance is closed on exit and dropped after a failure so the next query can open a fresh one.
- Pre-destruction partition snapshots are no longer deleted after 30 days. They were purged on age like any other snapshot, so the record of what a disk looked like before it was wiped, restored or cloned disappeared exactly as time made it valuable. They are now kept until 50 per disk accumulate; ad-hoc snapshots still age out. Retention reads the timestamp from the filename rather than the filesystem, which a copy or a restore resets.
- SMART history failures are no longer silent. An unreadable or unwritable history store meant trend alerts quietly stopped having anything to compare against, so a monitoring feature failed closed to "everything is fine". Read failures, unparseable files and failed quarantines are now logged.
- The CLI can now perform two of the operations a rescue actually needs: `pp clone --source N --dest N` and `pp wipe --disk N [--passes 1-7]`, both dry-run by default, requiring `--apply` and a typed `YES`, and both supporting `--json`. The WinPE rescue profile previously shipped a CLI that could inspect a disk but not act on one. Image capture and restore are still GUI-only.
...
- Fixed VHDX volume capture discarding all file security. Robocopy ran without `/COPYALL`, so every ACL, owner and audit entry was dropped and a restored image landed with inherited default permissions; it also followed junction points, and a Windows volume root carries one pointing back into itself. Capture now runs with `/COPYALL`, `/DCOPY:DAT`, `/XJ` and `/B`, and skips per-volume state that cannot be copied: System Volume Information, the recycle bin, and the page, hibernation and swap files. When the session lacks the Manage Auditing or Backup and Restore rights, robocopy refuses those switches and copies nothing at all, so capture retries with ACLs and ownership only and says so in the log rather than failing.
